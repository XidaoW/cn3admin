...
public class SyncContent {
	...
	public SyncContent(){
		...
		String sql = "SELECT contentID " + //,title,abstract,contentType,keywords " +
				"FROM cn3Testing_new_dump.content " +
				"WHERE contentID > 9374 AND conferenceID = " + conferenceID;//Import Poster from testing to Production
		...
		try {
			...
			while(rs.next()){
				...
				sql = "INSERT INTO content (title,abstract,contentType,paperTopic,contentTrack,keywords,conferenceID) " +
						"SELECT c.title,c.abstract,c.contentType,c.paperTopic,c.contentTrack,c.keywords,c.conferenceID FROM cn3Testing_new_dump.content c WHERE c.contentID=" + contentID;
				...
				if(productionContentID > 0){
					...
					sql = "INSERT INTO mapcontent(testingID, productionID) VALUES ("+contentID+","+productionContentID+")" ;
					conn.executeUpdate(sql);
